...
func (ctx *ProposalContext) Execute(txe *exec.TxExecution) error {
	...
	// Validate input
	proposeAcc, err := state.GetMutableAccount(ctx.StateWriter, proposal.Input.Address)
	if err != nil {
		return err
	}
	...
	if proposeAcc.Sequence()+1 != proposal.Input.Sequence {
	...
	for _, v := range proposal.Votes {
		...
		acc, err := state.GetMutableAccount(ctx.StateWriter, v.Address)
		if err != nil {
			return err
		}
